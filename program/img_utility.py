...
def show_img(img_list: list, size=(10,5)):
    ...
    plt.imshow(img_list, cmap='Greys_r')
    plt.show()
...
def crop_images(bounding_boxes: np.ndarray, img_list: list) -> list:
    ...
    for pts in coordinates:
        ...
        show_img(croped, size=(2,2))
        gray = cv2.cvtColor(croped, cv2.COLOR_RGB2GRAY)
#         show_img(gray, size=(2,2))
        total_mean_edge_colors = get_edge_colors(gray)
        total_mean_edge_colors_all.append(total_mean_edge_colors)     
        
#         th, gray = cv2.threshold(gray, 200, 255, cv2.THRESH_BINARY_INV)
        th, gray = cv2.threshold(gray, 200, 255, cv2.THRESH_BINARY)
#         show_img(croped, size=(2,2))
        croped_images.append(gray)
